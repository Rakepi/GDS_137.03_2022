...
function animate()
{
	...
	//--------------------------Place the bullet at the tip of the gun turret instead of at its center-------------------------
		//bullet.x = gun.x;
		//bullet.y = gun.y;
		//var gunD = Math.sqrt(gun.y*gun.x+target.y*target.x);
		//console.log(gunD);
		//bullet.x = gunD;
		//bullet.y = gunD;

		bullet.x = gun.x - Math.cos(radians)*gun.angle /*canvas.width/2 + Math.cos(gun.angle*Math.PI/180)*200*/;
		bullet.y = gun.y - Math.sin(radians)*gun.angle;
